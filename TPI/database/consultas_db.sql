...
BEGIN
 ...
 AND    (isnull(estadoId)
			OR (NOT isnull(estadoId) AND estadoId = 1 AND fecha_baja is not null )
            OR (NOT isnull(estadoId) AND estadoId = 2 AND fecha_baja is null)  )
...
BEGIN
UPDATE consulta 
SET consulta.cupo = consulta.cupo - 1 ,consulta.update_time = curdate()
